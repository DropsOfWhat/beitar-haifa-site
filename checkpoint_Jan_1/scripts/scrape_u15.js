...
const TARGET_URL = 'https://www.football.org.il/team-details/team-games/?team_id=3662&season_id=27';
const TEAM_ID_STR = 'team_id=3662';
const DB_PATH = path.join(__dirname, '../db.json');

async function scrapeU15() {
    console.log(`Starting U15 (Naarim G - Haim Toledano) scrape...`);
    ...
    try {
        ...
        const games = await page.evaluate(() => {
            ...
            rows.forEach(row => {
                ...
                if (matchCell) {
                    ...
                    if (parts.length >= 2) {
                        const isUs = (name) => name.includes('בית"ר חיפה') || name.includes('חיים טולדנו');
        ...
        console.log(`Scraped ${games.length} games for U15.`);
        ...
        if (teamIndex !== -1) {
            ...
            console.log(`Database updated for U15 (Index ${teamIndex}).`);
        } else {
            console.error(`Team with ID 3662 not found in DB!`);
        }
