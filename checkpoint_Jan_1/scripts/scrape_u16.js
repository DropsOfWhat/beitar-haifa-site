...
const TARGET_URL = 'https://www.football.org.il/team-details/team-games/?team_id=3346&season_id=27';
const TEAM_ID_STR = 'team_id=3346';
const DB_PATH = path.join(__dirname, '../db.json');

async function scrapeU16() {
    console.log(`Starting U16 (Naarim B) scrape...`);
    ...
    try {
        ...
        console.log(`Scraped ${games.length} games for U16.`);
        ...
        if (teamIndex !== -1) {
            ...
            // Optional: Rename if explicitly requested, but usually safer to keep original name unless told to rename in DB
            // db.teams[teamIndex].name = 'נערים ב - בית"ר חיפה'; 
            fs.writeFileSync(DB_PATH, JSON.stringify(db, null, 2));
            console.log(`Database updated for U16 (Index ${teamIndex}).`);
        } else {
            console.error(`Team with ID 3346 not found in DB!`);
        }
